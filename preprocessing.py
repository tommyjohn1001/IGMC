...
from data_utils import download_dataset, load_data, map_data
...
def globally_normalize_bipartite_adjacency(adjacencies, verbose=False, symmetric=True):
    ...
    # set zeros to inf to avoid dividing by zero
    degree_u[degree_u == 0.0] = np.inf
    degree_v[degree_v == 0.0] = np.inf

    degree_u_inv_sqrt = 1.0 / np.sqrt(degree_u)
    degree_v_inv_sqrt = 1.0 / np.sqrt(degree_v)
    ...
    if symmetric:
        adj_norm = [
            degree_u_inv_sqrt_mat.dot(adj).dot(degree_v_inv_sqrt_mat) for adj in adjacencies
        ]
...
def create_trainvaltest_split(
    ...
    if datasplit_from_file and os.path.isfile(datasplit_path):
        ...
        num_users, num_items, u_nodes, v_nodes, ratings, u_features, v_features = load_data(
            dataset, seed=seed, verbose=verbose
        )
    ...
    if post_rating_map is None:
        data = train_labels + 1.0
    else:
        data = np.array([post_rating_map[r] for r in class_values[train_labels]]) + 1.0
    data = data.astype(np.float32)

    rating_mx_train = sp.csr_matrix(
        (data, [u_train_idx, v_train_idx]), shape=[num_users, num_items], dtype=np.float32
    )
...
def load_data_monti(dataset, testing=False, rating_map=None, post_rating_map=None):
    ...
    if post_rating_map is None:
        ...
        rating_mx_train[train_idx] = (
            np.array([post_rating_map[r] for r in class_values[labels[train_idx]]]) + 1.0
        )
...
def load_official_trainvaltest_split(
    ...
    if dataset == "ml_100k":
        ...
        for movie_id, g_vec in zip(
            movie_df["movie id"].values.tolist(), movie_df[genre_headers].values.tolist()
        ):
