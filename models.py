...
class IGMC(GNN):
    ...
    def __init__(
        ...
        # FIXME: Khi mọi thứ đã oke, enable các dòng bên dưới
        ...
        #     sys.exit(1)
        # self.mlp_contrastive.load_state_dict(torch.load(path_weight_mlp))
    ...
    def forward(self, data, epoch=-1, is_training=True):
        ...
        pe = self.lin_pe(pe)
        x = self.mlp_contrastive(node_subgraph_feat)
